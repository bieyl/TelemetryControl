...
void BasicInfoEdit::insertInfo()
{
    ...
    QSqlQuery query(db);
    QString qs = QString("insert into missionbasicinfo (`时间`,`参谋`,`负责人`,`进场时间`,`开饭时间`,`联系电话`,`人员`)"
                         " values ('%1','%2','%3','%4','%5','%6','%7')").arg(ui->dateEdit->dateTime().toString("yyyy.MM.dd"))
            .arg(ui->comboBox->currentText()).arg(ui->comboBox_2->currentText()).arg(ui->timeEdit->dateTime().toString("hh:mm"))
            .arg(ui->timeEdit_2->dateTime().toString("hh:mm")).arg(ui->comboBox_3->currentText()).arg(people);
    qDebug() << qs;
